...
const hasExpandedStash = modList.some(m => m.id === 'ExpandedStash' && m.installed);
```
...
```javascript
const misc = D2RMM.readTsv('global\\excel\\misc.txt', { removeCarriageReturns: true });
...
```json
{
  ...
  "defaultValue": "normal",
  "options": [
    { "label": "Normal", "value": "normal", "description": "Vanilla drop rates" },

...
The GraphQL schema is publicly introspectable — no API key required. To get the full schema, send this curl request:

```bash
curl -s -X POST https://api.nexusmods.com/v2/graphql \
...
## D2RMM Mod Config Round-tripping

D2RMM embeds per-mod configs in a collection's `installationInfo` field so they can be restored when another user installs the collection.

**Write (upload)** — after `createCollection` / `createOrUpdateRevision` returns a `revisionId`:
...
The `installationInfo` string contains: `<d2rmm-configs>{"nexusModId": {...config...}}</d2rmm-configs>`
Keys are nexus mod IDs (numbers serialized as JSON string keys).

**Read (install)** — query `installationInfo` from `collectionRevision`:

```graphql
query CollectionRevisionInstallationInfo(
  $slug: String!
  $revision: Int!
  $viewAdultContent: Boolean
) {
...
Parse the `<d2rmm-configs>` tag from the returned string. Returns `{}` gracefully if not present.

The `installationInfo` field is displayed to users on the Nexus collection page as "Before you start" content.
